...
## واجهة الإضافة (أيقونة شريط الأدوات)

بالضغط على أيقونة الإضافة تفتح لوحة بثلاثة تبويبات + مفتاح تشغيل/إيقاف رئيسي:

- **البيانات** — عنوان المنافسة الملتقطة وعمرها ورقمها، ثم **معايير التقييم**
  كشجرة بأوزانها (مع وسم «مالي» للمعيار الذي سيذهب لجدول التقييم المالي الثابت،
  وتنبيه «سيُنشأ مستوى ثالث بنفس الاسم» للمعيار بلا فروع)، ثم **الحقول**
  المسطّحة (المفتاح ← القيمة) مع بحث فوري.
- **آخر تعبئة** — ملخص آخر صفحة عُبّئت (مُلئ/تُخطّي/فشل) وكل صف بتفصيله.
- **الإعدادات** — ثلاثة مفاتيح:
  | المفتاح | الافتراضي | الأثر |
  |---|---|---|
  | التعبئة التلقائية | مفعّل | إيقافه ⇐ الشريط يظهر لكن التعبئة تنتظر ضغط «تعبئة الحقول» |
  | شريط التحكم على الصفحة | مفعّل | إيقافه ⇐ لا إطار ولا شريط داخل اعتماد |
  | إضافة معايير التقييم | مفعّل | إيقافه ⇐ تعبئة الحقول المسطّحة فقط |
  وزر **مسح البيانات المحفوظة** (يمسح الحمولة وسجل التعبئة).

المفتاح الرئيسي في الترويسة يوقف الإضافة كليًا: لا التقاط من تنفيذ، ولا إطار
ولا تعبئة في اعتماد. تغيير أي إعداد ينعكس فورًا على صفحات اعتماد المفتوحة.
كل الإعدادات تُخزَّن محليًا في `chrome.storage.local` تحت `tanfeethSettings`.
...
```js
{
  ...
  tanfeethFillLog: {
    "/Tender/AddTender": {
      filled: 12, skipped: 3, failed: 0, at: 1769000005000,
      rows: [{ key: "TenderName", status: "filled", detail: "…" }]  // لعرضها في الواجهة
    }
  },

  // إعدادات المستخدم من واجهة الإضافة (كلها true افتراضيًا)
  tanfeethSettings: { enabled: true, autoFill: true, showOverlay: true, fillCriteria: true }
}
```
...
│   ├── etimad-criteria.js   # واجهة معايير التقييم المتكررة (المستوى ٢/٣ + الأوزان)
...
│   ├── popup.html/.css/.js  # واجهة الإضافة: الحالة والبيانات والسجل والإعدادات
